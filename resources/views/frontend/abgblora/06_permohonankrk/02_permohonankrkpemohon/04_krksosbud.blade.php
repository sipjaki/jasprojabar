     ...
     <div class="flex flex-col gap-5 w-full" style="margin-top: -25px;">
            <div class="bg-white flex flex-col gap-5 p-5 rounded-[20px] w-full">
                ...
                <p class="text-white font-normal text-sm">
                    <span class="font-bold">Form KRK <span style="color: #0fb825;"> Fungsi Sosial Budaya</span> Bangunan Gedung </span>
                </p>
            </div>
            <form id="signatureForm" action="{{ route('permohonan.krksosbudcreate') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-5 w-full" style="margin-top:-35px;">
                @csrf
...
<script>
    $('#kecamatanblora_id').on('change', function () {
        ...
        if (kecamatanID) {
            $.ajax({
                url: '{{ route("permohonan.krksosbud") }}', // Sesuaikan dengan route di controller
                type: 'GET',
