...
<button class="button-kembali" type="button"
    onclick="window.location.href='{{ url('bekonstruksiperhitunganbgn') }}';"
...
<br>
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                 <form action="{{ route('upload.bebantekupload6new', $data->id) }}" method="POST" enctype="multipart/form-data">
                       @csrf
                            <!-- begin::Body -->
                            <div class="card-body">
                                <div class="row">
...
<div class="col-md-6">
    <div class="mb-3">
        ...
        <input
            type="file"
            id="uploadsuratbantek"
            name="uploadsuratbantek"
            accept="application/pdf"
            class="form-control @error('uploadsuratbantek') is-invalid @enderror"
            onchange="previewPDF(event, 'pdfPreviewContainer', 'pdfPreview', 'uploadsuratbantekMessage')"
        />
        @error('uploadsuratbantek')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror

        {{-- Preview PDF jika sudah ada data --}}
        <div class="mt-3" id="pdfPreviewContainer"
             style="{{ isset($data->uploadsuratbantek) && $data->uploadsuratbantek ? '' : 'display: none;' }}">
            <iframe
                id="pdfPreview"
                src="{{ isset($data->uploadsuratbantek) ? asset($data->uploadsuratbantek) : '' }}"
        ...
        <div
            id="uploadsuratbantekMessage"
            class="mt-3"
            style="color: #666; font-style: italic; border: 1px solid #ddd; border-radius: 4px; padding: 20px; {{ isset($data->uploadsuratbantek) && $data->uploadsuratbantek ? 'display: none;' : 'display: block;' }}"
        >
            Silahkan Upload Berkas Bantuan Teknis
        </div>
    </div>
</div>
                                ...
                                <div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
                                    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                      <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">
                                        Apakah Anda ingin menerbitkan berkas ini ?
                                    </p>
