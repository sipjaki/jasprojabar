...
<br>
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                  <form action="{{ route('beperhitungankerusakanpernew', $data->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
                            <!-- begin::Body -->
                            <div class="card-body">
                                <div class="row">
...
<div class="mb-3">
    <label class="form-label" for="rab">
        <i class="bi bi-file-earmark-pdf" style="margin-right: 8px; color: darkred;"></i> Upload Rencana Anggaran Biaya (PDF)
    </label>
    <input
        type="file"
        id="rab"
        ...
        onchange="previewPDF(event, 'pdfPreviewContainerRab', 'pdfPreviewRab', 'uploadMessageRab')"
    />
    @error('rab')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror

    <div class="mt-3" id="pdfPreviewContainerRab"
         style="{{ isset($data->rab) && $data->rab ? '' : 'display: none;' }}">
        <iframe
            id="pdfPreviewRab"
            src="{{ isset($data->rab) ? asset($data->rab) : '' }}"
    ...
    <div
        id="uploadMessageRab"
        class="mt-3"
        style="color: #666; font-style: italic; border: 1px solid #ddd; border-radius: 4px; padding: 20px; {{ isset($data->rab) && $data->rab ? 'display: none;' : 'display: block;' }}"
    >
        Silahkan Upload Berkas Rencana Anggaran Biaya
    </div>
</div>

<div class="mb-3">
    <label class="form-label" for="asbuilt">
        <i class="bi bi-file-earmark-pdf" style="margin-right: 8px; color: darkred;"></i> Upload As Built Drawing (PDF)
    </label>
    <input
        type="file"
        id="asbuilt"
        name="asbuilt"
        accept="application/pdf"
        class="form-control @error('asbuilt') is-invalid @enderror"
        onchange="previewPDF(event, 'pdfPreviewContainerAsbuilt', 'pdfPreviewAsbuilt', 'uploadMessageAsbuilt')"
    />
    @error('asbuilt')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror

    <div class="mt-3" id="pdfPreviewContainerAsbuilt"
         style="{{ isset($data->asbuilt) && $data->asbuilt ? '' : 'display: none;' }}">
        <iframe
            id="pdfPreviewAsbuilt"
            src="{{ isset($data->asbuilt) ? asset($data->asbuilt) : '' }}"
            style="width: 100%; height: 400px; border: 1px solid #ddd; border-radius: 4px;"
        ></iframe>
    </div>

    <div
        id="uploadMessageAsbuilt"
        class="mt-3"
        style="color: #666; font-style: italic; border: 1px solid #ddd; border-radius: 4px; padding: 20px; {{ isset($data->asbuilt) && $data->asbuilt ? 'display: none;' : 'display: block;' }}"
    >
        Silahkan Upload Berkas As Built Drawing
    </div>
</div>
...
<div class="col-md-6">
    <div class="mb-3">
        ...
        <input
            ...
            onchange="previewPDF(event, 'pdfPreviewContainer1', 'pdfPreview1', 'uploadMessage1')"
        />
        @error('suratpermohonan')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror

        <div class="mt-3" id="pdfPreviewContainer1"
             style="{{ isset($data->suratpermohonan) && $data->suratpermohonan ? '' : 'display: none;' }}">
            <iframe
                id="pdfPreview1"
                src="{{ isset($data->suratpermohonan) ? asset($data->suratpermohonan) : '' }}"
                style="width: 100%; height: 400px; border: 1px solid #ddd; border-radius: 4px;"
            ></iframe>
        </div>
        ...
        <div
            id="uploadMessage1"
            class="mt-3"
            style="color: #666; font-style: italic; border: 1px solid #ddd; border-radius: 4px; padding: 20px; {{ isset($data->suratpermohonan) && $data->suratpermohonan ? 'display: none;' : 'display: block;' }}"
        >
    ...
    <div class="mb-3">
        ...
        <input
            ...
            name="kic"
            accept="application/pdf"
            class="form-control @error('kic') is-invalid @enderror"
            onchange="previewPDF(event, 'pdfPreviewContainer2', 'pdfPreview2', 'uploadMessage2')"
        />
        @error('kic')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror

        <div class="mt-3" id="pdfPreviewContainer2"
             style="{{ isset($data->kic) && $data->kic ? '' : 'display: none;' }}">
            <iframe
                id="pdfPreview2"
                src="{{ isset($data->kic) ? asset($data->kic) : '' }}"
                style="width: 100%; height: 400px; border: 1px solid #ddd; border-radius: 4px;"
            ></iframe>
        </div>
        ...
        <div
            id="uploadMessage2"
            class="mt-3"
            style="color: #666; font-style: italic; border: 1px solid #ddd; border-radius: 4px; padding: 20px; {{ isset($data->kic) && $data->kic ? 'display: none;' : 'display: block;' }}"
        >
        ...
        <input
            ...
            name="fotokondisi"
            accept="application/pdf"
            class="form-control @error('fotokondisi') is-invalid @enderror"
            onchange="previewPDF(event, 'pdfPreviewContainer3', 'pdfPreview3', 'uploadMessage3')"
        />
        @error('fotokondisi')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror

        <div class="mt-3" id="pdfPreviewContainer3"
             style="{{ isset($data->fotokondisi) && $data->fotokondisi ? '' : 'display: none;' }}">
            <iframe
                id="pdfPreview3"
                src="{{ isset($data->fotokondisi) ? asset($data->fotokondisi) : '' }}"
                style="width: 100%; height: 400px; border: 1px solid #ddd; border-radius: 4px;"
            ></iframe>
        </div>
        ...
        <div
            id="uploadMessage3"
            class="mt-3"
            style="color: #666; font-style: italic; border: 1px solid #ddd; border-radius: 4px; padding: 20px; {{ isset($data->fotokondisi) && $data->fotokondisi ? 'display: none;' : 'display: block;' }}"
        >
            Silahkan Upload Berkas Foto Kondisi
        </div>
    </div>
</div>
...
function previewPDF(event, previewContainerId, iframeId, messageId) {
    ...
    if (input.files && input.files[0]) {
        ...
        previewContainer.style.display = 'none';
        message.style.display = 'block';
        message.textContent = 'Silahkan Upload Berkas';
    }
}
</script>
                                ...
                                <div id="confirmModal" style="display: none; position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; justify-content: center; align-items: center;">
                                    <div style="background: white; padding: 24px 30px; border-radius: 12px; max-width: 400px; width: 90%; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                      <p style="font-size: 16px; font-weight: 600; margin-bottom: 20px;">
                                        Apakah Anda ingin memperbaiki berkas permohonan bantuan teknis anda ?
                                    </p>
